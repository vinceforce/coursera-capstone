...
vocabularyProcess <- function(train.sample, threshold = 1, exclude = FALSE) {
  ...
  dfm.1 <- quanteda::dfm(ngrams.1, tolower = TRUE)
...
vocabulary.1 <- parallelizeTask(vocabularyProcess, train.sample, 1, exclude = TRUE)
...
ngrams <- function(train.sample, what = "word", nbGram = 4, stem=FALSE, weight=NULL, smooth=NULL, skip=0, min_docfreq = 1, vocabulary, exclude = FALSE) {
  ...
  # terms_not_in_voc_q <- sapply(setdiff(tolower(terms), vocabulary$feature), function(t) paste0(" '", t, "' "))
  red <- function(x,y) {
    paste(x, y , sep = "|")
  }
...
ng.train <- computeProbs(ng.1.train, ng.2.train, ng.3.train, ng.4.train, vocabulary.1, del_unk = TRUE)
...
pairedngrams <- function(ngram, nbGram, stem = FALSE, pngram1 = NULL, k = 5) {
  if (nbGram == 1) {
    ...
    # ng.paired[pngram1, on=.(grams.n), freq.n:=i.gts.cnt]
    # ng.paired[, p.mle:=(gts.cnt / freq.n)]
    # # MLE NORMALISATION
    ...
    # ALPHA NORMALISATION
    # alpha.sum <- sum(ng.paired$alpha)
    # ng.paired[, alpha:=(alpha/alpha.sum)]
...
summary(png.2.train$alpha)
summary(png.3.train$alpha)
summary(png.4.train$alpha)
summary(png.2.train$sigma.k.n)
summary(png.3.train$sigma.k.n)
png.3.train[sigma.k.n==1]
summary(png.4.train$sigma.k.n)
summary(png.2.train$beta.k.n)
summary(png.3.train$beta.k.n)
summary(png.4.train$beta.k.n)
# summary(png.1.train$d.gts)
# summary(png.2.train$d.gts)
# summary(png.3.train$d.gts)
# summary(png.4.train$d.gts)
# summary(png.1.train$d.ktz)
# summary(png.2.train$d.ktz)
# summary(png.3.train$d.ktz)
# summary(png.4.train$d.ktz)
# 
...
png.1.train <- png.1.train[grams.0 != "END"]
png.2.train <- png.2.train[grams.0 != "END"]
png.3.train <- png.3.train[grams.0 != "END"]
png.4.train <- png.4.train[grams.0 != "END"]
...
vocabulary.1 <- data.table::fread("ngrams/bak_med/vocabulary.1.csv")
...
ngram.accuracy.all <- function(ngrams.test, d = d) {
  predict.dev.mat <- sapply(ngrams.test$grams.3, ngram.predict, nbrep = 10, d = 0.4, k = 5, preprocessinput = FALSE)
  df.res <- data.frame()
...
acc_test <- ngram.accuracy.all(png.4.testing.dev.sample, d = d)
...
png.1 <- png.1[!is.element(grams.0, profanity.words$V1)]
png.2 <- png.2[!is.element(grams.0, profanity.words$V1)]
png.3 <- png.3[!is.element(grams.0, profanity.words$V1)]
png.4 <- png.4[!is.element(grams.0, profanity.words$V1)]

# unknowns deletion
# png.1 <- png.1[grams.0 != "UNK"]
# png.2 <- png.2[grams.0 != "UNK"]
# png.3 <- png.3[grams.0 != "UNK"]
# png.4 <- png.4[grams.0 != "UNK"]

png.1 <- png.1[grams.0 != "END"]
png.2 <- png.2[grams.0 != "END"]
png.3 <- png.3[grams.0 != "END"]
png.4 <- png.4[grams.0 != "END"]


write.csv2(png.1, "pairedngrams/ng1-4.shiny_png.1.train.csv", row.names = FALSE)
saveRDS(png.1, "pairedngrams/ng1-4.shiny_png.1.train.rds")
write.csv2(png.2, "pairedngrams/ng1-4.shiny_png.2.train.csv", row.names = FALSE)
saveRDS(png.2, "pairedngrams/ng1-4.shiny_png.2.train.rds")
write.csv2(png.3, "pairedngrams/ng1-4.shiny_png.3.train.csv", row.names = FALSE)
saveRDS(png.3, "pairedngrams/ng1-4.shiny_png.3.train.rds")
write.csv2(png.4, "pairedngrams/ng1-4.shiny_png.4.train.csv", row.names = FALSE)
saveRDS(png.4, "pairedngrams/ng1-4.shiny_png.4.train.rds")
...
write.csv2(png.1, "pairedngrams/bak_big/ng1-4.shiny_png.1.train.csv", row.names = FALSE)
saveRDS(png.1, "pairedngrams/bak_big/ng1-4.shiny_png.1.train.rds")
write.csv2(png.2, "pairedngrams/bak_big/ng1-4.shiny_png.2.train.csv", row.names = FALSE)
saveRDS(png.2, "pairedngrams/bak_big/ng1-4.shiny_png.2.train.rds")
write.csv2(png.3, "pairedngrams/bak_big/ng1-4.shiny_png.3.train.csv", row.names = FALSE)
saveRDS(png.3, "pairedngrams/bak_big/ng1-4.shiny_png.3.train.rds")
write.csv2(png.4, "pairedngrams/bak_big/ng1-4.shiny_png.4.train.csv", row.names = FALSE)
saveRDS(png.4, "pairedngrams/bak_big/ng1-4.shiny_png.4.train.rds")
